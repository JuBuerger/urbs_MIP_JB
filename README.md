...
urbs MIP is an adaptation of urbs developed by the Chair of Renewable and Sustainable Energy Systems (see below). However, urbs MIP is no longer a linear optimisation model, but solves "mixed-integer programming (MIP)" problems.
This means that the processes in the input file can be defined not only linearly, but also discretely. Therefore, the "...-block" variable must be defined with a discrete quantity. 
The costs K must then be determined by a fixed price F and a variable price V, which is multiplied by the expansion quantity B (K=F+V*B).
The same procedure can be applied to storage technology.

To simulate a single input file, run_MIP.py must be executed and the associated input file must be stored in the folder "Input".
If several input files are to be simulated, they can be stored in the Input folder under "Simulation". These files are then simulated one after the other by run_MIP_Series.py.
